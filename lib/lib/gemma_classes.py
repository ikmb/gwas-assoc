...
class Gemma:
  """ class Gemma implements tasks on Gemma output files """
  
  def __init__(self, gemma_file, write_file=None, write_file_status_append=False):
    """ init """
    self._gemma_file = gemma_file    # name of gemma file
    ...
    self._snps_order = []      # order of SNPs from gemma file
    # only typed snps not imputed
    self._typed = {}
    # typed and/or imputed snps
    self._typed_imputed = {}
    
    self._snps_order_perchr = []      # order of SNPs from gemma file per chr, list of chr-lists
    # for each chr a list, index 0 -> chr0, index 1 -> chr1, ..., index 26 -> chr26
    for i in xrange(27):
      self._snps_order_perchr.append([])

  def map_gemma(self, typed=False):
    """ map p-values from GEMMA file into memory """
    try:
      fh = file(self._gemma_file,"r")
    except IOError, e:
      print e
      sys.exit(1)
  ...
  def free_map(self):
    """ free mapped GEMMA file from memory """
    self._snps_order       = []
    self._typed            = {}
    self._typed_imputed    = {}

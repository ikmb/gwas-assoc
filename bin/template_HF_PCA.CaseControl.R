Rplink <- function(PHENO,GENO,CLUSTER,COVAR){
	...
	wraper <- function(g, annotation, U){
		...
		#	for(eth in c("European", "African", "Asian")){
			#change: the ethnicity you want
                        #eth	<- "OtherAsian"
			eth	<- "European"

                        #changed DE
                        ####select <- annotation$affection=="Unaffected" & annotation$eth_pred ==eth
			#select <- annotation$phenotype=="1" & annotation$ethnicity_predicted ==eth
			#select <- annotation$phenotype==1 & annotation$ethnicity_predicted ==eth
			...
			i<-i+1
                        ...
                        #changed DE
                        #####select <- annotation$diag== "Crohn's Disease"& annotation$eth_pred ==eth
			select <- annotation$diagnosis!= "Control" & annotation$ethnicity_predicted ==eth
			ret <- adv_indep_anova_test(g, group, select, matched, U)
			pval[i] <- ret
			ret_batch <- adv_remove_one_test(g, group, select,batch, matched, U)
			...
			i<-i+1
                        ...
                        # auskommentiert DE
			#select <- annotation$diag== "Ulcerative Colitis"& annotation$eth_pred ==eth
			#ret <- adv_indep_anova_test(g, group, select, matched, U)
			#pval[i] <- ret
			#ret_batch <- adv_remove_one_test(g, group, select,batch, matched, U)
			#p_batch <- c(p_batch, ret_batch)

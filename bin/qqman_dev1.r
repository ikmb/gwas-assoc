...
qq = function(pvector,gridlines=F,gridlines.col='gray83',gridlines.lwd=1,gridlines.lty=1,confidence=T,confidence.col='gray81',
        # added DE
	pt.cex=0.5,pt.col='black',pt.bg='black',pch=21,abline.col='blue',abline.lwd=1.8,abline.lty=1,ymax=8,ymax.soft=F,title="",lambda="",lambda_1000="",numof.cases="",numof.controls="",
	...
	# added DE
	title(main=title)
	text(2,1,paste("lambda=", round(lambda, 3), "; lambda_1000=", round(lambda_1000, 3), "\nN(pvals)=", length(pvector), "\nN(ca)=", numof.cases, "; N(co)=", numof.controls, sep=""), cex=1.5, pos=4)
